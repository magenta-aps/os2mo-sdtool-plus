...
from sdtoolplus.autogenerated_graphql_client.input_types import EmployeeFilter, \
    AddressCreateInput, RAValidityInput, AddressUpdateInput, AddressFilter, ClassFilter
from sdtoolplus.config import TIMEZONE
from sdtoolplus.depends import GraphQLClient
from sdtoolplus.mo.timelines.common import mo_end_to_timeline_end, get_class, \
    timeline_interval_to_mo_validity
...
async def test_person_addresses(
    ...
    Phone (keep)      |-----12345678----|----------23456789-----------------------
    Phone (terminate) |---------------------45678901------------------------------
    Phone (terminate) |-------------10101010 (engagement_telefon_anden)-----------
    Email (ter+create)|---------chuck@karate.org----------|---chuck@judo.org------
    Email (keep)          ----------------norris@hollywood.com--------------------
    Email (terminate)     ----------------norris@hollywood.com--------------------
    Email (terminate)     ----------------chuck@hollywood.com---------|
    Email (terminate)                                     |---cn@hollywood.com----
    Postal (terminate)    --------------Paradisæblevej 13, 1000 Andeby------------

    Addresses in SD (no timeline!):
    Phone1: 23456789 (keep)
    Phone2: 34567890 (create)
    Email1: chuck@karate.org (term + create)
    Email2: norris@hollywood.com (keep)
    Postal: Fasanvænget 11, 2000 Gåserød (create)
    """
    ...
    t4 = datetime(2004, 1, 1, tzinfo=TIMEZONE)

    # person_uuid = uuid4()
    person_uuid = UUID("73474852-71fc-4948-b002-b1421ccb03a6")
    cpr = "0101011234"
    ...
    # Get the phone address type
    phone_address_type_uuid = await get_class(
    ...
    """
    respx_mock.get(f"https://service.sd.dk/sdws/GetPerson20111201?InstitutionIdentifier=II&EffectiveDate=01.07.2002&PersonCivilRegistrationIdentifier={cpr}&StatusActiveIndicator=True&StatusPassiveIndicator=True&ContactInformationIndicator=True&PostalAddressIndicator=True").respond(
        content_type="text/xml;charset=UTF-8",
        content=sd_resp,
    )
    ...
    phone_numbers = await graphql_client.get_address_timeline(
        input=AddressFilter(
            ...
            address_type=ClassFilter(uuids=[phone_address_type_uuid]),
            from_date=None,
            to_date=None,
        )
    )

    phone_number = one(phone_numbers.objects)

    interval_1 = phone_number.validities[0]
    assert interval_1.validity.from_ == t1
    ...
    assert interval_1.address_type.uuid == phone_address_type_uuid

    interval_2 = phone_number.validities[1]
    assert interval_2.validity.from_ == t1
    assert mo_end_to_timeline_end(interval_2.validity.to) == t2
    assert interval_2.user_key == "23456789"
    assert interval_2.value == "23456789"
    assert interval_2.visibility_uuid == visibility_uuid
    assert interval_2.address_type.uuid == phone_address_type_uuid

    assert len(phone_number.validities) == 2
