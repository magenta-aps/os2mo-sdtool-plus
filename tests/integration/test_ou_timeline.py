...
from sdtoolplus.mo.timeline import _get_mo_validity
...
async def test_ou_timeline_name_and_id_and_level_and_parent_http_triggered_sync(
    ...
    assert _from_mo_end_datetime(validity.validity.to) == t2
    ...
    assert _from_mo_end_datetime(validity.validity.to) == t3
    assert validity.name == "name1"
    assert validity.user_key == "ABCD"
    assert validity.org_unit_level is not None
    ...
    assert _from_mo_end_datetime(validity.validity.to) == t5
    ...
    assert _from_mo_end_datetime(validity.validity.to) == t7
...
async def test_ou_timeline_sd_unit_should_extend_mo_unit(
    ...
    await graphql_client.create_org_unit(
        OrganisationUnitCreateInput(
            uuid=unit_uuid,
            validity=_get_mo_validity(start=t2, end=t6),
            name="name1",
            user_key="ABCD",
            parent=OrgUnitUUID("10000000-0000-0000-0000-000000000000"),
            org_unit_type=org_unit_type,
    ...
    validity = validities[0]
    assert validity.validity.from_ == t1
    assert _from_mo_end_datetime(validity.validity.to) == POSITIVE_INFINITY
    assert validity.name == "name1"
    assert validity.user_key == "ABCD"
    assert validity.org_unit_level is not None
    assert validity.org_unit_level.name == "NY0-niveau"
