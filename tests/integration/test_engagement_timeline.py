...
@pytest.mark.envvar(
    {
        "APPLY_NY_LOGIC": "false",
        "ENABLE_ELEVATED_MANAGERS": "true",
    }
)
async def test_eng_timeline_elevated_managers(
    test_client: AsyncClient,
    graphql_client: GraphQLClient,
    base_tree_builder: TestingCreateOrgUnitOrgUnitCreate,
    job_function_1234: UUID,
    ...
    """
    We are testing the "elevated engagements for managers" scenario. In this
    case, we must ensure that an engagement unit is not changed if the employee
    of the engagement is also the manager in the same unit.

    In this test, we are triggering engagement 12345, but the employee also has
    engagement 23456, which is placed in the same unit (E) as 12345 in one of
    the intervals to ensure that this engagement is not modified (since we are
    only triggering engagement 12345). The employee has two (disjoint) manager
    objects.

    Time  --------t1---t2---t3---------t4------t5----t6---t7---------t9-------->

    MO (12345)    |--dep1---|---dep2---|       |---dep2---|---dep4---|
    MO (23456)    |------------------------dep5---------------------------------
    MO manager 1  |----------------dep2--------------|------dep4-----|
    MO manager 2                                                     |---dep5---

    SD (12345)         |-----dep1------|       |---dep3---|-------dep5----------

    Desired(12345)     |-d1-|---dep2---|       |-dep2|-d3-|---dep4---|---dep5---
    Desired(23456)|------------------------dep5---------------------------------
    ...
    # Update engagement 12345 (t5 -> t7)
    await graphql_client.update_engagement(
        EngagementUpdateInput(
            uuid=eng_uuid_12345,
            user_key=emp_id,
            validity=timeline_interval_to_mo_validity(t5, t7),
            extension_1="prof1",
            extension_4="dep2",
            extension_5=str(dep2_uuid),
            person=person_uuid,
    ...
    # Create engagement 23456 (t1 -> inf)
    eng_uuid_23456 = (
        await graphql_client.create_engagement(
            EngagementCreateInput(
                user_key="23456",
    ...
    # Manager classes
    r_responsibility = await graphql_client.get_class(
        ClassFilter(user_keys=["Lederansvar"])
    )
    responsibility = one(r_responsibility.objects).uuid

    r_manager_type = await graphql_client.get_class(
        ClassFilter(user_keys=["SectionHead"])
    )
    manager_type = one(r_manager_type.objects).uuid

    r_manager_level = await graphql_client.get_class(ClassFilter(user_keys=["Level1"]))
    manager_level = one(r_manager_level.objects).uuid

    # Create manager object 1
    manager1_uuid = (
        await graphql_client._testing__create_manager(
            ManagerCreateInput(
                ...
                validity=timeline_interval_to_mo_validity(t1, t6),
            )
        )
    ).uuid

    # Update manager object 1
    await graphql_client._testing__update_manager(
        ManagerUpdateInput(
            ...
            manager_level=manager_level,
            manager_type=manager_type,
            validity=timeline_interval_to_mo_validity(t6, t9),
        )
    )

    # Create manager object 2
    await graphql_client._testing__create_manager(
        ManagerCreateInput(
            person=person_uuid,
            responsibility=[responsibility],
          ...
          <Person>
            ...
            <Employment>
              ...
              <EmploymentDepartment>
                ...
                <DepartmentUUIDIdentifier>{str(dep5_uuid)}</DepartmentUUIDIdentifier>
              </EmploymentDepartment>
              <Profession>
                <ActivationDate>2002-01-01</ActivationDate>
                <DeactivationDate>9999-12-31</DeactivationDate>
                <JobPositionIdentifier>1234</JobPositionIdentifier>
                <EmploymentName>prof1</EmploymentName>
                <AppointmentCode>0</AppointmentCode>
              </Profession>
              <EmploymentStatus>
                <ActivationDate>2002-01-01</ActivationDate>
    ...
    assert interval_1.validity.from_ == t2
    assert _mo_end_to_timeline_end(interval_1.validity.to) == t3
    assert interval_1.extension_1 == "prof1"
    assert interval_1.extension_4 == "dep1"
    assert interval_1.extension_5 == str(dep1_uuid)
    assert interval_1.user_key == emp_id
    assert interval_1.job_function_uuid == job_function_1234
