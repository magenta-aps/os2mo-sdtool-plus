...
def test_timeline_engagement_type():
    # Arrange
    intervals = (
        EngagementType(
            start=datetime(2025, 6, 10, tzinfo=ZoneInfo("Europe/Copenhagen")),
    ...
    # Act
    eng_type_timeline = Timeline[EngagementType](intervals=intervals)
    timeline = EngagementTimeline(eng_type=eng_type_timeline)

    # Assert
    assert eng_type_timeline.intervals == intervals
    assert timeline.eng_type == eng_type_timeline
