...
@click.command()
@click.option(
    "--use-test-env", is_flag=True, help="If we need to use the special SD test URLs"
)
def main(use_test_env: bool) -> None:
    ...
    sd_persons = asyncio.run(
        missing_persons(
            gql_client=gql_client, settings=settings, use_test_env=use_test_env
        )
    )
