...
from .create_person_address import CreatePersonAddress
from .create_person_address import CreatePersonAddressAddressCreate
...
from .terminate_person_address import TerminatePersonAddress
from .terminate_person_address import TerminatePersonAddressAddressTerminate
...
from .update_person_address import UpdatePersonAddress
from .update_person_address import UpdatePersonAddressAddressUpdate
...
class GraphQLClient(AsyncBaseClient):
    ...
    async def get_person_timeline(self, cpr_numbers: Any) -> GetPersonTimelineEmployees:
        query = gql(
            """
            query GetPersonTimeline($cpr_numbers: CPR!) {
              employees(filter: {cpr_numbers: [$cpr_numbers], to_date: null, from_date: null}) {
                objects {
    ...
    async def update_person(
        ...
        query = gql(
            """
            mutation UpdatePerson($uuid: UUID!, $cpr_number: CPR!, $given_name: String!, $surname: String!, $from: DateTime!, $to: DateTime = null) {
              employee_update(
                input: {validity: {from: $from, to: $to}, cpr_number: $cpr_number, given_name: $given_name, surname: $surname, uuid: $uuid}
              ) {
                uuid
              }
            }
    ...
    async def create_person_address(
        ...
        from_: datetime,
        to: Union[Optional[datetime], UnsetType] = UNSET,
    ) -> CreatePersonAddressAddressCreate:
        query = gql(
            """
            mutation CreatePersonAddress($person: UUID!, $value: String!, $address_type: UUID!, $visibility: UUID!, $from: DateTime!, $to: DateTime = Null) {
              address_create(
                input: {validity: {from: $from, to: $to}, value: $value, address_type: $address_type, visibility: $visibility, person: $person}
              ) {
                uuid
              }
            }
        ...
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return CreatePersonAddress.parse_obj(data).address_create
    ...
    async def update_person_address(
        ...
        from_: datetime,
        to: Union[Optional[datetime], UnsetType] = UNSET,
    ) -> UpdatePersonAddressAddressUpdate:
        query = gql(
            """
            mutation UpdatePersonAddress($uuid: UUID!, $value: String!, $address_type: UUID!, $visibility: UUID!, $from: DateTime!, $to: DateTime = Null) {
              address_update(
                input: {uuid: $uuid, validity: {from: $from, to: $to}, address_type: $address_type, value: $value, visibility: $visibility}
              ) {
                uuid
              }
            }
        ...
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return UpdatePersonAddress.parse_obj(data).address_update

    async def terminate_person_address(
        self, uuid: UUID, to: datetime
    ) -> TerminatePersonAddressAddressTerminate:
        query = gql(
            """
        ...
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return TerminatePersonAddress.parse_obj(data).address_terminate
