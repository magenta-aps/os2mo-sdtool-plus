...
class GraphQLClient(AsyncBaseClient):
    ...
    async def get_org_unit_engagements(
        self,
        ou_filter: Union[Optional[OrganisationUnitFilter], UnsetType] = UNSET,
        eng_filter: Union[Optional[EngagementFilter], UnsetType] = UNSET,
    ) -> GetOrgUnitEngagementsOrgUnits:
        query = gql(
            """
            query GetOrgUnitEngagements($ou_filter: OrganisationUnitFilter, $eng_filter: EngagementFilter) {
              org_units(filter: $ou_filter) {
                objects {
                  validities {
                    engagements(filter: $eng_filter) {
                      user_key
                      person {
                        cpr_number
                      }
        ...
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return GetOrgUnitEngagements.parse_obj(data).org_units
