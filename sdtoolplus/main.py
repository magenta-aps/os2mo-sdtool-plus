...
def create_fastramqpi(**kwargs: Any) -> FastRAMQPI:
    ...
    async def timeline_sync_person(
        ...
        sd_person = one(sd_response.Person)

        # sd_eng_timeline = await get_person_timeline(r_person)

        # Get the person
        mo_person = await gql_client.get_person(payload.cpr)
        person = only(mo_person.objects)
        if person is None:
            logger.info("create person", person_info=sd_person)
            employee_input = EmployeeCreateInput(
                cpr_number=sd_person.PersonCivilRegistrationIdentifier,
                given_name=sd_person.PersonGivenName,
                surname=sd_person.PersonSurnameName,
            )
            await gql_client._testing__create_person(input=employee_input)
