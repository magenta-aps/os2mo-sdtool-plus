...
def create_fastramqpi(**kwargs: Any) -> FastRAMQPI:
    ...
    async def timeline_sync_person(
        ...
        contact_information = sd_response_person.ContactInformation
        sd_post = sd_response_person.PostalAddress
        sd_person = Person(
            ...
            emails=contact_information.EmailAddressIdentifier
            if contact_information
            else [],
            phone_numbers=contact_information.TelephoneNumberIdentifier
            if contact_information
            else [],
            addresses=f"{sd_post.StandardAddressIdentifier}, {sd_post.PostalCode}, {sd_post.DistrictName}"
            if sd_post
            else [],
        )
        ...
        ## todo: remove:
        assert person
        person_addresses = await gql_client.get_addresses(
            filter=AddressFilter(employee={"uuids": [person.uuid]})
        )
        objects = person_addresses.objects
        mo_emails = [a for v in objects for a in v]
        breakpoint()
        if sd_person.emails:
            if not person_addresses:
                logger.info("create")

            if sd_person.emails == person_addresses:
