...
def sd_start_datetime(d: date) -> datetime:
    return datetime.combine(d, time.min, ASSUMED_SD_TIMEZONE)


def sd_end_datetime(d: date) -> datetime:
    if d == date.max:
...
async def get_department_timeline(
    ...
    active_intervals = tuple(
        Active(
            start=sd_start_datetime(dep.ActivationDate),
            end=sd_end_datetime(dep.DeactivationDate),
            value=True,
        )
    ...
    parent_intervals = tuple(
        UnitParent(
            start=sd_start_datetime(parent.startDate),
            end=sd_end_datetime(parent.endDate),
...
async def get_employment_timeline(
    ...
    active_intervals = (
        tuple(
            Active(
                start=sd_start_datetime(status.ActivationDate),
                end=sd_end_datetime(status.DeactivationDate),
                value=True,
            )
    ...
    eng_key_intervals = (
        tuple(
            EngagementKey(
                start=sd_start_datetime(prof.ActivationDate),
                end=sd_end_datetime(prof.DeactivationDate),
    ...
    eng_type_intervals = (
        tuple(
            EngagementType(
                start=sd_start_datetime(working_time.ActivationDate),
                end=sd_end_datetime(working_time.DeactivationDate),
