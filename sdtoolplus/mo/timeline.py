...
def _get_ou_address(
    ...
    return only(
        ...
        if address.address_type.name == address_type_name
    )
...
async def get_ou_timeline(
    ...
    postal_address_intervals = tuple(
        UnitPostalAddress(
            ...
            value=_get_ou_address(obj.addresses, "Postadresse fra SD"),
        )
        for obj in validities
        if _get_ou_address(obj.addresses, "Postadresse fra SD") is not None
    )

    phone_number_intervals = tuple(
        UnitPhoneNumber(
            start=obj.validity.from_,
            end=_mo_end_to_timeline_end(obj.validity.to),
            value=_get_ou_address(obj.addresses, "Telefonnummer"),
        )
        for obj in validities
        if _get_ou_address(obj.addresses, "Postadresse fra SD") is not None
    )
    ...
    p_number_intervals = tuple(
        UnitPNumber(
            ...
            value=_get_ou_address(obj.addresses, "P-nummer"),
        )
        for obj in validities
        if _get_ou_address(obj.addresses, "P-nummer") is not None
    )
...
async def create_ou(
    ...
    # Create the OU
    payload_ou = OrganisationUnitCreateInput(
        uuid=org_unit,
    ...
    logger.debug("OU create payload", payload=payload_ou.dict())
    if not dry_run:
        await gql_client.create_org_unit(payload_ou)

    # Create the OU addresses

    # TODO: place these class calls somewhere during application start up and pass them
    #       to a context for better performance

    # Get the address visibility UUID
    visibility_class_uuid = await _get_facet_class(
        gql_client=gql_client,
    ...
    postal_address_type_uuid = await _get_facet_class(
        gql_client=gql_client,
    ...
    try:
        payload_postal_address = AddressCreateInput(
            org_unit=org_unit,
            visibility=visibility_class_uuid,
            validity=timeline_interval_to_mo_validity(start, end),
            user_key=desired_unit_timeline.postal_address.entity_at(start).value,
            value=desired_unit_timeline.postal_address.entity_at(start).value,
            address_type=postal_address_type_uuid,
        )
        logger.debug(
            "OU postal address create payload",
            payload_postal_address=payload_postal_address.dict(),
        )
        if not dry_run:
            await gql_client.create_address(payload_postal_address)
    except NoValueError:
        logger.info("No address found for unit", org_unit=str(org_unit))

    # Get the postal address type
    phone_address_type_uuid = await _get_facet_class(
        gql_client=gql_client,
        facet_user_key="org_unit_address_type",
        # TODO: use correct class user_key in municipality mode
    ...
    try:
        payload_phone = AddressCreateInput(
            org_unit=org_unit,
            visibility=visibility_class_uuid,
            validity=timeline_interval_to_mo_validity(start, end),
            user_key=desired_unit_timeline.phone.entity_at(start).value,
            value=desired_unit_timeline.phone.entity_at(start).value,
            address_type=phone_address_type_uuid,
        )
        logger.debug(
            "OU phone number create payload",
            payload_phone=payload_phone,
        )
        if not dry_run:
            await gql_client.create_address(payload_phone)
    except NoValueError:
        logger.info("No phone number found for unit", org_unit=str(org_unit))

    # Get the postal address type
    p_number_address_type_uuid = await _get_facet_class(
        gql_client=gql_client,
        facet_user_key="org_unit_address_type",
        # TODO: use correct class user_key in municipality mode
    ...
    try:
        payload_p_number = AddressCreateInput(
            org_unit=org_unit,
            visibility=visibility_class_uuid,
            validity=timeline_interval_to_mo_validity(start, end),
            user_key=desired_unit_timeline.p_number.entity_at(start).value,
            value=desired_unit_timeline.p_number.entity_at(start).value,
            address_type=p_number_address_type_uuid,
        )
        logger.debug(
            "OU P-number create payload",
            payload_p_number=payload_p_number,
        )
        if not dry_run:
            await gql_client.create_address(payload_p_number)
    except NoValueError:
        logger.info("No P-number found for unit", org_unit=str(org_unit))
