...
async def related_units(
    ...
    logger.warn("got endpoints", endpoints)
    ...
    for start, end in pairwise(endpoints):
        ...
        endpoints = {start, end}
        logger.warn("finding sub-validities of validity", start=start, end=end)
        for obj in mo_parents.objects:
            for val in obj.validities:
                if val.validity.from_ > start:
                    endpoints.add(val.validity.from_)
                val_end = mo_end_to_timeline_end(val.validity.to)
                if val_end < end:
                    endpoints.add(val_end)

        for start, end in pairwise(sorted(endpoints)):
            logger.warn("sub-validity", start=start, end=end)
