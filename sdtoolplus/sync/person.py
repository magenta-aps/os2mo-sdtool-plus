...
async def _sync_address(
    ...
    # There can only one of each address type. If there are more than one,
...
async def _sync_engagement_addresses(
    ...
    for eng_address in engagement_addresses:
        ...
        try:
            object_ = one(
                engagement_timeline.objects, too_long=MoreThanOneEngagementError
            )
        except ValueError:
            logger.error(
                "Engagement not found. Cannot sync person address",
                address=eng_address.dict(),
            )
