...
from respx import MockRouter, ASGIHandler
...
@pytest.fixture
def sd_mock(respx_mock: MockRouter) -> Callable[[str, Any], None]:
    """Allow tests to fake the SD API."""

    def _sd_mock(endpoint: str, params: dict, api_response: Any) -> None:
        app = fake_sd_api.create_app(endpoint=endpoint, api_response=api_response)
        respx_mock.route(
            host="service.sd.dk", path="/sdws/", params=params
        ).mock(side_effect=ASGIHandler(app))

    # Start mocking an empty SD instance immediately in case the integration makes calls
    # before the test configures its mocks.
    _sd_mock("", [])
    return _sd_mock
...
async def test_client(asgiapp: ASGIApp) -> AsyncIterator[AsyncClient]:
    ...
    transport = ASGITransport(app=asgiapp, client=("1.2.3.4", 123))
