...
from sdtoolplus.mo.timeline import create_engagement, related_units
...
async def engagement_ou_strategy_region(
    ...
    sd_timeline = sd_ou_only_timeline.eng_unit
    mo_timeline = mo_ou_only_timeline.eng_unit
    for start, end in pairwise(endpoints):
        if mo_eng_timeline.has_value(start):
            placement_units = related_units(
                gql_client=gql_client,
                start=start,
                end=end,
                lookup_unit=mo_ou_only_timeline.eng_unit.entity_at(start).value,
            )
