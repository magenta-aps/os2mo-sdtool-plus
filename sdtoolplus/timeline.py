...
from sdtoolplus.autogenerated_graphql_client import ClassFilter, ManagerFilter
...
from sdtoolplus.mo.timeline import create_association, _mo_end_to_timeline_end
...
async def engagement_ou_strategy_managers(
    gql_client: GraphQLClient,
    person: UUID,
    sd_eng_timeline: EngagementTimeline,
    mo_eng_timeline: EngagementTimeline,
    ...
    1) If the person related to the engagement is a manager in a given period,
       use the already registered MO unit of the engagement in this period. If
       the engagement does not already have a MO unit (which it should!), use
       the SD unit as a fallback.
    2) If the person is not a manager in a given period, use the SD unit.
    """
    logger.info("Applying OU manager strategy")
    ...
    endpoints = sorted(sd_interval_endpoints.union(mo_interval_endpoints).union(manager_endpoints))
    logger.debug("List of manager and engagement unit endpoints", endpoints=endpoints)
