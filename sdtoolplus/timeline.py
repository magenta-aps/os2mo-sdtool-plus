...
async def engagement_ou_strategy_elevate_to_ny_level(
    ...
    # Find the OU parent timelines (using sync HTTP calls to spare the SD API)
    ...
    for eng_unit_uuid in eng_unit_uuids:
        try:
            ...
            ou_parent_timelines[eng_unit_uuid] = Timeline[UnitParent](
                intervals=combine_intervals(parent_intervals)
            )
        except SDParentNotFound as error:
            logger.error(
                "Error getting department parent(s) from SD. Cannot elevate engagement to the above NY-level(s)",
                error=error,
